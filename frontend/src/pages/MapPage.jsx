...
export default function MapPage() {
    ...
    const animalMap = useRef(new Map());
    useEffect(() => {
    ...
    const animalName = (id) => animalMap.current.get(id)?.name ?? id.slice(0, 8) + '…';
    const animalSpecies = (id) => animalMap.current.get(id)?.species ?? '—';
    return (<div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 56px - 3rem)' }}>
      {/* ── Toolbar ─────────────────────────────────────────────────────── */}
      <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', marginBottom: '0.75rem', flexWrap: 'wrap' }}>
        <h1 style={{ color: '#34d399', margin: 0, fontSize: '1.5rem' }}>Sightings Map</h1>

        <label style={{ marginLeft: 'auto' }}>
          Animal&nbsp;
          <select value={filterAnimal} onChange={e => { setFilterAnimal(e.target.value); }} style={{ padding: '5px', borderRadius: '4px', border: '1px solid #3a3a3a', background: '#141414', color: '#e8e8e8' }}>
        ...
        <label>
          Status&nbsp;
          <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} style={{ padding: '5px', borderRadius: '4px', border: '1px solid #3a3a3a', background: '#141414', color: '#e8e8e8' }}>
      ...
      {error && <p style={{ color: '#f87171', margin: '0 0 0.5rem' }}>{error}</p>}

      {/* ── Legend ──────────────────────────────────────────────────────── */}
      <div style={{ display: 'flex', gap: '1rem', marginBottom: '0.5rem', flexWrap: 'wrap' }}>
        {STATUSES.map(s => (<span key={s} style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.8rem' }}>
      ...
      {/* ── Map ─────────────────────────────────────────────────────────── */}
      <div style={{ flex: 1, borderRadius: '8px', overflow: 'hidden', border: '1px solid #333333' }}>
        <MapContainer center={DEFAULT_CENTER} zoom={DEFAULT_ZOOM} style={{ height: '100%', width: '100%' }}>
          <TileLayer attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a>' url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"/>

          {reports.map(r => (<CircleMarker key={r.id} center={[r.location.latitude, r.location.longitude]} radius={8} pathOptions={{
...
function ReportPopup({ report: r, name, species }) {
    ...
    return (<div style={{ fontSize: '0.85rem', minWidth: '180px' }}>
